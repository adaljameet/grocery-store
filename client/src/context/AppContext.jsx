...
export const AppContextProvider = ( {children} ) => {
   ...
   const [products, setProducts] = useState([]);
    ...
    const fetchSeller = async () => {
        try {
            const { data } = await axios.get('http://localhost:8000/api/seller/is-auth');
            if(data.success){
                setIsSeller(true);
            }else{
                setIsSeller(false);
    ...
    const fetchUser = async () => {
        try {
            const { data } = await axios.get('http://localhost:8000/api/user/is-auth');
            if(data.success){
                setUser(data.user);
   ...
   const fetchProducts = async () => {
    ...
    useEffect(() => {
        const updateCart = async () => {
            try {
                const { data } = await axios.post('http://localhost:8000/api/cart/update', {cartItems});
                if(!data.success){
                    toast.error(data.message);
                }
            } catch (error) {
