...
function SellerLayout() {
    ...
    const logout = async () => {
        try {
            const { data } = await axios.get('http://localhost:8000/api/seller/logout');
            if(data.success){
    ...
    <>
        <div className="flex items-center justify-between px-4 md:px-8 border-b border-gray-300 py-3 bg-white">
            ...
            <div className="flex items-center gap-5 text-gray-500">
                <p>Hi! Admin</p>
                <button onClick={logout} className='border rounded-full text-primary-dull px-4 py-1 cursor-pointer'>Logout</button>
            </div>
        </div>
        <div className='flex'>
