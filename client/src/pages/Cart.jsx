...
const Cart = () => {
    ...
    const placeOrder = async () => {
        try {
            ...
            if(paymentOption === 'COD'){
                const { data } = await axios.post('http://localhost:8000/api/order/cod', {
                    userId: user._id,
            ...
            // Place order with Stripe
                ...
                const { data } = await axios.post('http://localhost:8000/api/order/stripe', {
                    userId: user._id,
                    items: cartArray.map(item => ({product: item._id, quantity: item.quantity})),
                    address: selectedAddress._id
                });
    ...
    const getUserAddress = async () => {
        try {
            const { data } = await axios.get('http://localhost:8000/api/address/get');
            if(data.success){
